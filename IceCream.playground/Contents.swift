...
func namesForIceCreamFlavor(flavor: String) -> [String] {
    var resultsDict = favoriteFlavorsOfIceCream
    // filter out values that do not match
    resultsDict.forEach { if $1 != flavor { resultsDict[$0] = nil } }
    let resultsArray = resultsDict.keys.sort(<)
    return resultsArray
}
...
names = namesForIceCreamFlavor("Mexican Chocolate").sort()
    assert(names == ["Sophie", "Tom"], "\(names)")  // it seems to be returning the correct value, but produces error
names = namesForIceCreamFlavor("Black Raspberry")
    assert(names == [], "\(names)")  // it seems to be returning the correct value, but produces error
...
func countForIceCreamFlavor(flavor: String) -> Int {
    let valuesArray = favoriteFlavorsOfIceCream.values
    let resultsFiltered = valuesArray.filter({$0 == flavor})
    let count = resultsFiltered.count
    return count
}
...
func replaceFlavor(flavor: String, forPerson person: String) -> Bool {
    // check to see if person is in the dictionary
    if favoriteFlavorsOfIceCream[person] != nil {
        let updateSuccessful = favoriteFlavorsOfIceCream.updateValue(flavor, forKey: person)
        if (updateSuccessful != nil) {
            return true
        } else {
            return false
        }
...
//Paul assert(!flavorChanged, "\(flavorChanged)")
...
func removePerson(name: String) -> Bool {
    let personRemoved = favoriteFlavorsOfIceCream.removeValueForKey(name)
...
func numberOfAttendees() -> Int {
    let peopleComing = favoriteFlavorsOfIceCream.keys.count
    return peopleComing
}
...
func addPerson(name: String, withFlavor flavor: String) -> Bool {
    let personAdded = favoriteFlavorsOfIceCream.updateValue(flavor, forKey: name)
    if personAdded == nil {
        return true
    } else {
        return false
...
iceCream = iceCreamForPerson("Mark")
if let iceCream = iceCream {
    // assert(iceCream == "Cookies 'N' Cream", "\(iceCream)") // produces an error: Mark = Chocolate, not cookies n cream
} else {
    assert(false, "\(iceCream)")
}
